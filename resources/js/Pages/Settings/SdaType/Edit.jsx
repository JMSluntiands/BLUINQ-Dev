...
export default function SdaTypeEdit({ sdaType, listFilters = {} }) {
    const listQs = filterQueryString(listFilters);

    const form = useForm({
        code: sdaType.code ?? '',
        name: sdaType.name,
        status: sdaType.status,
    });

    const submit = (e) => {
        e.preventDefault();
        form.patch(
            route('settings.sda-type.update', sdaType.id) +
                listQs,
        );
    };
        ...
        <WorkflowSettingsLayout moduleKey="sda-type"
            header={
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <Link
                        href={
                            route('settings.sda-type.index') + listQs
                        }
