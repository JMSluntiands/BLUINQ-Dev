...
const FLASH_MESSAGES = {
    'workflow-status-created': 'Status created.',
    'workflow-status-updated': 'Status updated.',
    'workflow-status-archived': 'Status moved to archive.',
};

export default function WorkflowStatusIndex({
    workflowStatuses,
    filters = {},
    kindOptions = {},
}) {
    const rows = workflowStatuses?.data ?? [];
    ...
    const confirmArchive = useCallback(() => {
        ...
        router.delete(
            route('settings.workflow-status.destroy', archiveTarget.id) + qs,
            { preserveScroll: true },
        );
            ...
            header={
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <Link
                            href={route('settings.workflow-status.create')}
                        ...
                        <Link
                            href={
                                route('settings.workflow-status.archive') +
            ...
            <div className="space-y-4">
                <div className="overflow-hidden rounded-2xl border border-[#e6e9ef] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
                    <TableSearchToolbar
                        ...
                        ziggyRouteName="settings.workflow-status.index"
                        filters={filters}
                    />
                    <DataTable
                        data={rows}
                    ...
                    />
                    <Pagination pagination={workflowStatuses} />
                </div>
            </div>
        </WorkflowSettingsLayout>
    );
