...
export default function ServiceEngagingEdit({ serviceEngaging, listFilters = {} }) {
    const listQs = filterQueryString(listFilters);

    const form = useForm({
        name: serviceEngaging.name,
        status: serviceEngaging.status,
    });

    const submit = (e) => {
        e.preventDefault();
        form.patch(
            route('settings.service-engaging.update', serviceEngaging.id) +
                listQs,
        );
    };
    ...
    return (
            ...
            header={
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-slate-800">
                        Service engaging — Edit
                    </h2>
                    <Link
                        href={
                            route('settings.service-engaging.index') + listQs
                        }
        ...
        >
            <Head title="Service engaging — Edit" />
