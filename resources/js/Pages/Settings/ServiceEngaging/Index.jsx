...
const FLASH_MESSAGES = {
    'service-engaging-created': 'Service engaging created.',
    'service-engaging-updated': 'Service engaging updated.',
    'service-engaging-archived': 'Service engaging moved to archive.',
};

export default function ServiceEngagingIndex({ serviceEngagings, filters = {} }) {
    const rows = serviceEngagings?.data ?? [];
    ...
    const confirmArchive = useCallback(() => {
        ...
        router.delete(
            route('settings.service-engaging.destroy', archiveTarget.id) + qs,
            { preserveScroll: true },
        );
                    ...
                    return (
                        <div className="flex flex-wrap items-center justify-end gap-0.5">
                            <Link
                                href={
                                    route(
                                        'settings.service-engaging.edit',
                                        r.id,
                                    ) + q
                                }
                                className={iconBtn}
    ...
    return (
        <WorkflowSettingsLayout moduleKey="service-engaging"
            header={
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-[#323338]">
                        Service engaging — List
                    </h2>
                    <div className="flex flex-wrap gap-2">
                        <Link
                            href={route('settings.service-engaging.create')}
                        ...
                        >
                            Add service engaging
                        </Link>
                        <Link
                            href={
                                route('settings.service-engaging.archive') +
        ...
        >
            <Head title="Service engaging — List" />
            ...
            <div className="space-y-4">
                <div className="overflow-hidden rounded-2xl border border-[#e6e9ef] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
                    <TableSearchToolbar
                        ...
                        ziggyRouteName="settings.service-engaging.index"
                        filters={filters}
                    />
                    <DataTable
                        data={rows}
                        ...
                        emptyMessage={
                            hasSearch
                                ? 'No service engaging entries match your search.'
                                : 'No service engaging entries yet. Add one to get started.'
                        }
                    />
                    <Pagination pagination={serviceEngagings} />
                </div>
            </div>
