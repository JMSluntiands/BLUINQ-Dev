...
const FLASH_MESSAGES = {
    'arrival-input-file-created': 'Arrival input file created.',
    'arrival-input-file-updated': 'Arrival input file updated.',
    'arrival-input-file-archived': 'Arrival input file moved to archive.',
};

export default function ArrivalInputFilesIndex({
    arrivalInputFiles,
    filters = {},
}) {
    const rows = arrivalInputFiles?.data ?? [];
    ...
    const confirmArchive = useCallback(() => {
        ...
        router.delete(
            route(
                'settings.crm.arrival-input-files.destroy',
                archiveTarget.id,
            ) + qs,
            { preserveScroll: true },
        );
                    ...
                    return (
                        <div className="flex flex-wrap items-center justify-end gap-0.5">
                            <Link
                                href={
                                    route(
                                        'settings.crm.arrival-input-files.edit',
                                        r.id,
                                    ) + q
                                }
                                className={iconBtn}
    ...
    return (
        <WorkflowSettingsLayout moduleKey="arrival-input-files"
            header={
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-[#323338]">
                        Arrival input files — List
                    </h2>
        ...
        >
            <Head title="Arrival input files — List" />
            ...
            <div className="space-y-4">
                <div className="overflow-hidden rounded-2xl border border-[#e6e9ef] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
                    <TableSearchToolbar
                        ...
                        ziggyRouteName="settings.crm.arrival-input-files.index"
                        filters={filters}
                    />
                    <DataTable
                        data={rows}
                        ...
                        emptyMessage={
                            hasSearch
                                ? 'No arrival input files match your search.'
                                : 'No arrival input files yet. Add one to get started.'
                        }
                    />
                    <Pagination pagination={arrivalInputFiles} />
                </div>
            </div>
