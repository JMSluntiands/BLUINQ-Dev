...
export default function BuildingClassEdit({ buildingClass, listFilters = {} }) {
    const listQs = filterQueryString(listFilters);

    const form = useForm({
        code: buildingClass.code ?? '',
        name: buildingClass.name,
        status: buildingClass.status,
    });

    const submit = (e) => {
        e.preventDefault();
        form.patch(
            route('settings.building-class.update', buildingClass.id) +
                listQs,
        );
    };
    ...
    return (
        <WorkflowSettingsLayout moduleKey="building-class"
            header={
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <Link
                        href={
                            route('settings.building-class.index') + listQs
                        }
        ...
        >
            <Head title="Building class — Edit" />
