...
const FLASH_MESSAGES = {
    'storey-level-created': 'Typical storey & level created.',
    'storey-level-updated': 'Typical storey & level updated.',
    'storey-level-archived': 'Typical storey & level moved to archive.',
};

export default function StoreyLevelIndex({ storeyLevels, filters = {} }) {
    const rows = storeyLevels?.data ?? [];
    ...
    const confirmArchive = useCallback(() => {
        ...
        router.delete(
            route('settings.storey-level.destroy', archiveTarget.id) + qs,
            { preserveScroll: true },
        );
    ...
    return (
        <WorkflowSettingsLayout moduleKey="storey-level"
            header={
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-[#323338]">
                        Typical storeys & levels — List
                    </h2>
                    <div className="flex flex-wrap gap-2">
                        <Link
                            href={route('settings.storey-level.create')}
                        ...
                        <Link
                            href={
                                route('settings.storey-level.archive') +
        ...
        >
            <Head title="Typical storeys & levels — List" />
            ...
            <div className="space-y-4">
                <div className="overflow-hidden rounded-2xl border border-[#e6e9ef] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
                    <TableSearchToolbar
                        ...
                        ziggyRouteName="settings.storey-level.index"
                        filters={filters}
                    />
                    <DataTable
                        data={rows}
                    ...
                    />
                    <Pagination pagination={storeyLevels} />
                </div>
            </div>
