...
export default function StoreyLevelEdit({ storeyLevel, listFilters = {} }) {
    const listQs = filterQueryString(listFilters);

    const form = useForm({
        code: storeyLevel.code ?? '',
        name: storeyLevel.name,
        status: storeyLevel.status,
    });

    const submit = (e) => {
        e.preventDefault();
        form.patch(
            route('settings.storey-level.update', storeyLevel.id) +
                listQs,
        );
    };
    ...
    return (
            ...
            header={
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-slate-800">
                        Typical storeys & levels — Edit
                    </h2>
                    <Link
                        href={
                            route('settings.storey-level.index') + listQs
                        }
        ...
        >
            <Head title="Typical storeys & levels — Edit" />
