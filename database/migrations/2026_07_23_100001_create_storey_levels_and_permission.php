...
return new class extends Migration
{
    public function up(): void
    {
        if (! Schema::hasTable('storey_levels')) {
            Schema::create('storey_levels', function (Blueprint $table) {
                $table->id();
                $table->string('name');
        ...
        $slug = 'settings.storey-level.view';
        ...
        $payload = [
            'name' => 'Settings — Typical storeys & levels',
            'status' => 'active',
            'sort_order' => 50,
            'updated_at' => $now,
        ];
    ...
    {
        Schema::dropIfExists('storey_levels');
        ...
        $id = DB::table('permissions')
            ->where('slug', 'settings.storey-level.view')
            ->value('id');
        if ($id) {
